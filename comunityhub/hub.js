...
//handling image and converting to base 64 format so that it can display and use on browser direclty
let convertedImage = "";
$("#profile_image").on("change", function () {
  ...
  if (imgs) {
    ...
    reader.onload = function () {
      ...
      $("#image-preview").html(
        `<img src="${reader.result}" alt="Image Preview" style="max-width: 200px;">`
      );
    };
...
function validateTextLength() {
  ...
  comentTextBox.on("input", function () {
    ...
    if (currCommentLength > maxChar) {
      ...
      CharDisplayNo.text(
        `Exceeded by ${currCommentLength - maxChar} characters!`
      ).css("color", "#F8AFAF");
    } else {
      comentTextBox.css("border", "");
      CharDisplayNo.text(
        `${maxChar - currCommentLength} characters remaining`
      ).css("color", "#000000");
    }
  });
}
  ...
  const previewData = `
        <div class="testimonial-card">
            <div class="testimonial-header">
                <img src="${
                  convertedImage || "default-avatar.png"
                }" alt="Profile Image" class="testimonial-image">
                <div class="testimonial-info">
                    <h4>${firstName} ${lastName}</h4>
                    <div class="rating">${"⭐".repeat(totalRating)}${"☆".repeat(
    5 - totalRating
  )}</div>
                </div>
            </div>
...
function displayTestimonialData(
